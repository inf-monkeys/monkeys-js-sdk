...
test('cursor window has one declared event owner independent from its query target', () => {
  ...
  const parsed = ResolvedPageSchema.safeParse(value);
  assert.equal(parsed.success, true, parsed.error?.message);
  assert.equal(ResolvedPageSchema.safeParse({ ...value, ontologyBindings: [{ ...binding, cursorWindow: { ...binding.cursorWindow, sourceCapabilityInstanceId: 'missing' } }] }).success, false);
  assert.equal(ResolvedPageSchema.safeParse({ ...value, actionBindings: [{ ...page.actionBindings[0], source: { capabilityInstanceId: source.instanceId, port: 'page.change' } }] }).success, false);
  assert.equal(ResolvedPageSchema.safeParse({ ...value, ontologyBindings: [binding, { ...binding, bindingId: 'duplicate-owner' }] }).success, false);
});
