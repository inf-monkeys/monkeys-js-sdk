...
`@inf-monkeys/monkeys` 是 Monkeys 跨仓 TypeScript 合同、运行时 Schema 与 JSON Schema 的唯一发布源。所有消费者只使用一套 canonical 合同和稳定公开入口，禁止导入包内 `src/*`。
...
- `@inf-monkeys/monkeys/contracts`：canonical TypeScript 合同。
