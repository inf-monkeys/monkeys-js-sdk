...
export interface DeclarativeCapabilityRegistration {
  ...
  allowedSideEffects: readonly ('network' | 'storage' | 'navigation' | 'clipboard' | 'worker' | 'websocket')[];
}
...
export const sameRevisionRef = (left: RevisionRef, right: RevisionRef): boolean =>
  revisionRefKey(left) === revisionRefKey(right);
...
const sha256Hex = (value: string): string => {
  ...
  for (let offset = 0; offset < source.length; offset += 64) {
    for (let index = 0; index < 16; index += 1) {
      ...
      words[index] =
        ((source[start] << 24) | (source[start + 1] << 16) | (source[start + 2] << 8) | source[start + 3]) >>> 0;
    }
...
export const declarativeRuntimeProjectionId = (
  ...
  releaseRevisionRef: Pick<RevisionRef, 'kind' | 'id' | 'ownerRepo' | 'revision' | 'schemaVersion' | 'contentHash'>,
): string => `drb:${canonicalContentHash({ resourceKind, releaseSlotId, releaseRevisionRef })}`;
...
export const declarativeRouteOwnerIndexProjectionId = (
  ...
  `dri:${canonicalContentHash({
    tenantScope: {
      teamRef: tenantScope.teamRef ? { kind: tenantScope.teamRef.kind, id: tenantScope.teamRef.id } : null,
    },
    environmentRef,
    surface,
  })}`;
...
const takeoverHashSource = (authorization: Omit<LegacyRouteTakeoverAuthorization, 'contentHash'>) => authorization;
...
export const parseExactLegacyRouteTakeoverAuthorization = (input: unknown): LegacyRouteTakeoverAuthorization => {
...
export const permissionAlternativePolicyAllows = (
  ...
  const available = permissionCodes instanceof Set ? permissionCodes : new Set(permissionCodes);
  return parsed.alternatives.some((alternative) => alternative.every((permission) => available.has(permission)));
};
...
export const compileDeclarativeWorkbenchCatalog = (input: {
  ...
  const entries = input.entries
    .map((entry) => ({ ...entry }))
    .sort((left, right) => left.order - right.order || left.workbenchId.localeCompare(right.workbenchId));
  const unsigned = {
...
export const declarativeRouteSpaceSourceHash = (routeSpace: DeclarativeRouteSpaceRegistration['routeSpace']): string =>
  canonicalContentHash(routeSpace);
...
export const compileDesignCapabilityCatalogArtifact = (input: {
  ...
  entries.forEach((entry, index) => {
    ...
    if (
      ...
      registration.capabilityRevisionRef.ownerRepo !== entry.manifest.ownerRepo ||
      String(registration.capabilityRevisionRef.revision) !== String(entry.manifest.capabilityVersion) ||
      registration.capabilityRevisionRef.contentHash !== canonicalContentHash(entry.manifest) ||
      ...
      String(registration.providerRevisionRef.revision) !== String(entry.provider.providerVersion) ||
      registration.providerRevisionRef.contentHash !== canonicalContentHash(entry.provider) ||
    ...
    registration.schemaDocuments.forEach((schema, schemaIndex) => {
      if (
        ...
        schema.schemaRevisionRef.contentHash !== canonicalContentHash(schema.document)
      ) {
        throw new DeclarativeControlCompilationError(
          'DESIGN_CATALOG_INVALID',
...
export const parseExactProductDeclarativeCapabilityRegistration = (
  ...
  if (
    registration.capabilityRevisionRef.contentHash !== canonicalContentHash(entry.manifest) ||
    registration.providerRevisionRef.contentHash !== canonicalContentHash(entry.provider) ||
    registration.sourceContentHash !== canonicalContentHash(unsignedRegistration) ||
  ...
  registration.schemaDocuments.forEach((schema, schemaIndex) => {
    if (schema.schemaRevisionRef.contentHash !== canonicalContentHash(schema.document)) {
      throw new DeclarativeControlCompilationError(
        'PRODUCT_CAPABILITY_CATALOG_INVALID',
...
export const parseExactDesignCapabilityCatalogArtifact = (input: unknown): DesignCapabilityCatalogArtifact => {
...
const dependencyKey = (role: ReleaseDependency['role'], reference: RevisionRef) =>
  `${role}:${revisionRefKey(reference)}`;
const requireDependencies = (snapshot: readonly ReleaseDependency[], required: readonly ReleaseDependency[]) => {
...
const assertDependencyScopes = (snapshot: readonly ReleaseDependency[], scope: TenantScope) =>
...
export const accessPolicyChainImplies = (policyInputs: readonly AccessPolicy[], targetInput: AccessPolicy): boolean => {
  ...
  if (!chainImpliesAny(policies, new Set(target.permissionAnyOf), permissionAll, (policy) => policy.permissionAnyOf))
    return false;
  ...
  if (!chainImpliesAny(policies, refSet(target.groupAnyOf), groupAll, (policy) => policy.groupAnyOf.map(stableRefKey)))
    return false;
...
const acceptedTrailingSlashStates = (
  ...
  return new Set([claim.normalizedPath.length > 1 && claim.normalizedPath.endsWith('/')]);
};
const routeSegmentParameter = (segment: string) => segment.match(/^[:$]([A-Za-z][A-Za-z0-9_-]*)$/)?.[1];
const parameterTypeFor = (claim: Pick<CompiledRouteClaim, 'matcher'>, name: string) =>
  claim.matcher.parameters.find((parameter) => parameter.name === name)?.type;
export const compiledRouteClaimsOverlap = (leftInput: CompiledRouteClaim, rightInput: CompiledRouteClaim): boolean => {
  const left = CompiledRouteClaimSchema.parse(leftInput);
  const right = CompiledRouteClaimSchema.parse(rightInput);
  if (![...acceptedTrailingSlashStates(left)].some((state) => acceptedTrailingSlashStates(right).has(state)))
    return false;
  ...
  return leftSegments.every((leftSegment, index) => {
    ...
    if (!leftParameter && !rightParameter) {
      if (left.matcher.caseSensitive && right.matcher.caseSensitive) return leftSegment === rightSegment;
    ...
    return !(leftType === 'integer' && rightType === 'uuid') && !(leftType === 'uuid' && rightType === 'integer');
  });
};
export const compileRouteClaims = (input: CompileRouteClaimsInput): readonly CompiledRouteClaim[] => {
  const embeddedTakeovers = input.claims.flatMap((claim) => {
    const authorization =
      'legacyRouteTakeoverAuthorization' in claim ? claim.legacyRouteTakeoverAuthorization : undefined;
    return authorization ? [authorization] : [];
  });
  const takeoverAuthorizations = [...(input.legacyRouteTakeoverAuthorizations ?? []), ...embeddedTakeovers].map(
    (authorization, index) => {
      try {
  ...
  const routeSpacesByRevision = new Map(
    registrations.map((entry) => [revisionRefKey(entry.revisionRef), entry.routeSpace]),
  );
  const compiled: CompiledRouteClaim[] = [];
  input.claims.forEach((inputClaim, index) => {
    const { legacyRouteTakeoverAuthorization: _authorization, ...runtimeClaim } = inputClaim as DeclarativeRouteClaim;
    const legacyClaim =
      'surface' in runtimeClaim ? RouteClaimSchema.parse(runtimeClaim) : LegacyRouteClaimSchema.parse(runtimeClaim);
    const routeSpace = routeSpacesByRevision.get(revisionRefKey(legacyClaim.routeSpaceRevisionRef));
    if (!routeSpace)
      throw new DeclarativeControlCompilationError(
        'ROUTE_SPACE_MISSING',
    ...
    const claim = RouteClaimSchema.parse({
      ...legacyClaim,
      surface: 'surface' in legacyClaim ? legacyClaim.surface : routeSpace.supportedSurface,
    });
    ...
    if (reserved) {
      const takeover = takeoverAuthorizations.find(
        (authorization) =>
          ...
          sameRevisionRef(authorization.routeSpaceRevisionRef, compiledClaim.routeSpaceRevisionRef),
      );
      if (!takeover) {
        throw new DeclarativeControlCompilationError(
          'ROUTE_RESERVED',
    ...
    const conflict = compiled.find(
      (candidate) =>
        sameRevisionRef(candidate.routeSpaceRevisionRef, claim.routeSpaceRevisionRef) &&
 ...
 */
export const matchCompiledRouteClaim = (claimInput: CompiledRouteClaim, pathInput: string): CompiledRouteMatch => {
  ...
  if (rawSegments.some((segment) => segment === '.' || segment === '..')) return { matched: false, parameters: {} };
  const requestHasTrailingSlash = rawPath.length > 1 && rawPath.endsWith('/');
  if (claim.matcher.trailingSlash === 'require' && !requestHasTrailingSlash) return { matched: false, parameters: {} };
  if (
    claim.matcher.trailingSlash === 'preserve' &&
    requestHasTrailingSlash !== (claim.normalizedPath.length > 1 && claim.normalizedPath.endsWith('/'))
  ) {
    return { matched: false, parameters: {} };
  }
  const normalizeTrailing = (value: string) => {
    if (claim.matcher.trailingSlash === 'remove' && value.length > 1) return value.replace(/\/+$/, '');
    if (claim.matcher.trailingSlash === 'require' && value.length > 1 && !value.endsWith('/')) return `${value}/`;
    return value;
  };
  ...
  if (requestSegments.length !== templateSegments.length) return { matched: false, parameters: {} };
  const parameterDefinitions = new Map(claim.matcher.parameters.map((parameter) => [parameter.name, parameter]));
  ...
  for (let index = 0; index < templateSegments.length; index += 1) {
    ...
    if (!definition || !routeParameterPattern(definition.type).test(value)) return { matched: false, parameters: {} };
    parameters[definition.name] = value;
    normalizedSegments.push(rawValue);
  }
  const canonicalPath = normalizeTrailing(`/${normalizedSegments.join('/')}` || '/');
  return {
    matched: true,
...
function routeParameterPattern(type: RouteSpace['parameters'][number]['type']): RegExp {
...
const domainQueryDependencies = (definition: DomainQueryDefinition): ReleaseDependency[] => [
  ...(definition.ontologyDefinitionRevisionRef
    ...
    : []),
  ...(definition.viewRevisionRef ? [{ role: 'view' as const, revisionRef: definition.viewRevisionRef }] : []),
...
export const compilePageRuntimeBundle = (input: CompilePageRuntimeBundleInput): PageRuntimeBundle => {
  const parsedPage = PageSchema.parse(input.page);
  const page = {
    ...parsedPage,
  ...
  const queryRegistrations = (input.domainQueryRegistry ?? []).map((registration, index) => {
    const definitionRevisionRef = RevisionRefSchema.parse(registration.definitionRevisionRef);
    const definition = DomainQueryDefinitionSchema.parse(registration.definition);
    assertSameTenantScope(definition.tenantScope, scope, `domainQueryRegistry[${index}].definition.tenantScope`);
    ...
    if (definitionRevisionRef.contentHash !== canonicalContentHash(definition)) {
      throw new DeclarativeControlCompilationError(
        'QUERY_DEFINITION_MISMATCH',
  ...
  assertUniqueRegistryKeys(
    queryRegistrations.map((registration) => revisionRefKey(registration.definitionRevisionRef)),
    'domainQueryRegistry',
  );
  ...
  page.queryBindings.forEach((binding, index) => {
    const definition = queryDefinitionsByRevision.get(revisionRefKey(binding.queryDefinitionRevisionRef));
    if (!definition) {
      throw new DeclarativeControlCompilationError(
        'QUERY_DEFINITION_MISMATCH',
    ...
    if (!sameRevisionRef(definition.resultSchemaRevisionRef, binding.renderModelSchemaRevisionRef)) {
      throw new DeclarativeControlCompilationError(
        'QUERY_DEFINITION_MISMATCH',
    ...
    if (
      !accessPolicyChainImplies([page.pageAccessPolicy], definition.accessPolicy) &&
    ...
    requireDependencies(release.dependencySnapshot, domainQueryDependencies(definition));
  });
  const shellRegistration = DeclarativeProductResourceRegistrationSchema.parse(input.shellRegistration);
  if (
    shellRegistration.resourceKind !== 'shell' ||
    !sameRevisionRef(shellRegistration.resourceRevisionRef, page.shellRevisionRef) ||
    declarativeProductResourceRevisionHash(shellRegistration) !== shellRegistration.resourceRevisionRef.contentHash ||
    declarativeProductResourceSourceHash(shellRegistration.document) !== shellRegistration.sourceContentHash
  ) {
    throw new DeclarativeControlCompilationError(
      'SHELL_REVISION_MISMATCH',
      'shellRegistration',
  ...
  const shellDocument = DeclarativeShellResourceDocumentSchema.parse(shellRegistration.document);
  const shellChrome = shellDocument.surfaceChrome.find((entry) => entry.surface === release.target.surface);
  if (!shellChrome) {
    throw new DeclarativeControlCompilationError(
      'SHELL_CHROME_UNAVAILABLE',
  ...
  page.capabilityInstances.forEach((instance, index) => {
    ...
    if (
      !node ||
      !compareLegacyEntityRef(node.capabilityRef, instance.capabilityRevisionRef) ||
      !node.providerRef ||
    ...
    if (!sameRevisionRef(instance.propertySchemaRevisionRef, registration.propertySchemaRevisionRef)) {
      throw new DeclarativeControlCompilationError(
        'CAPABILITY_BINDING_MISMATCH',
    ...
    if (
      !accessPolicyChainImplies(
        [page.pageAccessPolicy, ...(instance.accessPolicy ? [instance.accessPolicy] : [])],
  ...
  page.ontologyBindings.forEach((binding, index) => {
    const instance = page.capabilityInstances.find(
      (candidate) => candidate.instanceId === binding.target.capabilityInstanceId,
    );
    const registration =
      instance &&
    ...
    if (binding.cursorWindow) {
      ...
      if (
        !pageChangePort ||
        !sameRevisionRef(pageChangePort.schemaRevisionRef, binding.cursorWindow.pageChangeIntentSchemaRevisionRef)
      ) {
        throw new DeclarativeControlCompilationError(
          'PORT_TYPE_MISMATCH',
  ...
  page.interactionBindings.forEach((binding, index) => {
    const instance = page.capabilityInstances.find(
      (candidate) => candidate.instanceId === binding.source.capabilityInstanceId,
    );
    const registration =
      instance &&
      registrationsByPair.get(
  ...
  const routeClaims = compileRouteClaims({
    ...
    legacyRouteTakeoverAuthorizations: release.target.legacyRouteTakeoverAuthorization
      ? [release.target.legacyRouteTakeoverAuthorization]
      : [],
  });
  const releaseRoute = routeClaims.find(
    (claim) => claim.kind === 'canonical' && claim.normalizedPath === release.target.normalizedPath,
  );
  ...
  const routeTargets = (input.targetRegistry ?? []).map((registration, index) => ({
    ...registration,
    ...
    ...(registration.releaseRevisionRef
      ? {
          releaseRevisionRef: RevisionRefSchema.parse(registration.releaseRevisionRef),
        }
      : {}),
  ...
  assertUniqueRegistryKeys(
    routeTargets.map((registration) => stableRefKey(registration.stableTargetRef)),
    'targetRegistry',
  );
  const entryTransitions = page.entryTransitions.map((transition, index) => {
    ...
    if (target.releaseRevisionRef && !['page-release', 'workbench-release'].includes(target.releaseRevisionRef.kind)) {
      throw new DeclarativeControlCompilationError(
        'PAGE_ENTRY_TARGET_UNRELEASED',
        `entryTransitions[${index}].targetRef`,
    ...
    const requiredPathParameters = [...target.routeClaim.normalizedPath.matchAll(/:([A-Za-z][A-Za-z0-9_-]*)/g)].map(
      (match) => match[1]!,
    );
    const mappedPathParameters = Object.keys(transition.pathParameters);
    const missing = requiredPathParameters.find((name) => !mappedPathParameters.includes(name));
    const unknown = mappedPathParameters.find((name) => !requiredPathParameters.includes(name));
    if (missing || unknown) {
      throw new DeclarativeControlCompilationError(
        'ROUTE_SPACE_MISMATCH',
    ...
    requireDependencies(release.dependencySnapshot, [
      ...
      ...(target.releaseRevisionRef
        ? [
            {
              role: target.releaseRevisionRef.kind as 'page-release' | 'workbench-release',
    ...
    return {
      ...
      ...(target.releaseRevisionRef ? { targetReleaseRevisionRef: target.releaseRevisionRef } : {}),
...
export const compileWorkbenchRuntimeBundle = (input: CompileWorkbenchRuntimeBundleInput): WorkbenchRuntimeBundle => {
  ...
  assertUniqueRegistryKeys(
    targetRegistry.map((registration) => stableRefKey(registration.stableTargetRef)),
    'targetRegistry',
  );
  const targetsByStableRef = new Map(
    targetRegistry.map((registration) => [stableRefKey(registration.stableTargetRef), registration]),
  );
  ...
  const unsigned = {
    ...
    identity: {
      ...
      ...(workbench.identity.description ? { description: workbench.identity.description } : {}),
    },
    surface: release.target.surface,
    routeClaims,
...
const orderedNavigationNodes = (navigation: Navigation): Navigation['nodes'] => {
  ...
  children.forEach((siblings) =>
    siblings.sort((left, right) => left.order - right.order || left.nodeId.localeCompare(right.nodeId)),
  );
...
export const compileNavigationRuntimeBundle = (input: CompileNavigationRuntimeBundleInput): NavigationRuntimeBundle => {
  ...
  if (
    release.target.surface !== navigation.supportedSurfaces.find((surface) => surface === release.target.surface) ||
  ...
  const nodes = orderedNavigationNodes(navigation).map((node, index) => {
    ...
    if (
      ...
      (registration.kind === 'route' &&
        (Boolean(registration.releaseRevisionRef) !== Boolean(released.releaseRevisionRef) ||
          (registration.releaseRevisionRef &&
            released.releaseRevisionRef &&
            !sameRevisionRef(registration.releaseRevisionRef, released.releaseRevisionRef))))
    ) {
      throw new DeclarativeControlCompilationError(
        'NAV_TARGET_UNRELEASED',
    ...
    if (registration.kind === 'route') {
      ...
      try {
        input = JsonObjectSchema.parse(
          registration.validateInput ? registration.validateInput(mappedInput) : mappedInput,
        );
      } catch (error) {
        throw new DeclarativeControlCompilationError(
          'NAV_ACTION_INPUT_INVALID',
      ...
      const sourceDependency = release.dependencySnapshot.find(
        (dependency) =>
          ...
          sameRevisionRef(dependency.revisionRef, registration.sourceCatalogRevisionRef),
      );
      if (!sourceDependency)
        throw new DeclarativeControlCompilationError(
          'DEPENDENCY_MISSING',
      ...
      const inputDependency = release.dependencySnapshot.find(
        (dependency) =>
          dependency.role === 'schema' && sameRevisionRef(dependency.revisionRef, registration.inputSchemaRevisionRef),
      );
      const resultDependency = registration.resultSchemaRevisionRef
        ? release.dependencySnapshot.find(
            (dependency) =>
              dependency.role === 'schema' &&
              sameRevisionRef(dependency.revisionRef, registration.resultSchemaRevisionRef!),
          )
        : undefined;
...
export const validateWorkbenchRuntimeSet = (
  ...
  return Object.freeze(
    [...bundles].sort(
      (left, right) => left.target.order - right.target.order || left.workbenchId.localeCompare(right.workbenchId),
    ),
  );
};
