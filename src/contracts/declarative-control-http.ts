...
import { DomainCommandDefinitionSchema, SchemaDefinitionSchema } from './semantic';
...
export const DeclarativeLegacyMigrationKindSchema = z.enum(['page', 'page-data-assets', 'workbench']);
export type DeclarativeLegacyMigrationKind = z.infer<typeof DeclarativeLegacyMigrationKindSchema>;
...
export const declarativeControlRoutes = {
  ...
  authoringHistory: (kind: DeclarativeResourceKind, resourceId: string): string =>
  ...
  migrationInspect: (kind: DeclarativeLegacyMigrationKind, legacyResourceId: string): string =>
    `${DECLARATIVE_CONTROL_API_PREFIX}/authoring/migrations/${kind}/${encodePath(legacyResourceId)}/inspect`,
  migrationCreateDraft: (kind: DeclarativeLegacyMigrationKind, legacyResourceId: string): string =>
    `${DECLARATIVE_CONTROL_API_PREFIX}/authoring/migrations/${kind}/${encodePath(legacyResourceId)}/draft`,
  migrationCutover: (kind: DeclarativeLegacyMigrationKind, targetResourceId: string): string =>
    `${DECLARATIVE_CONTROL_API_PREFIX}/authoring/resources/${kind === 'workbench' ? 'workbench' : 'page'}/${encodePath(targetResourceId)}/publish`,
  migrationRollback: (kind: DeclarativeLegacyMigrationKind, targetResourceId: string): string =>
...
const forbiddenRuntimeParameterKeys = new Set(['__proto__', 'prototype', 'constructor']);
const rejectRuntimePrototypeKeys = (value: unknown, context: z.RefinementCtx, path: PropertyKey[] = []): void => {
  if (Array.isArray(value)) {
    value.forEach((child, index) => rejectRuntimePrototypeKeys(child, context, [...path, index]));
    return;
  }
  ...
  for (const key of Object.keys(value)) {
    ...
    rejectRuntimePrototypeKeys((value as Record<string, unknown>)[key], context, [...path, key]);
  }
};
const runtimeParameterDepth = (value: unknown, depth = 0): number => {
  ...
  return values.reduce((maximum, child) => Math.max(maximum, runtimeParameterDepth(child, depth + 1)), depth);
};
    ...
    JsonObjectSchema.superRefine((value, context) => {
      ...
      if (keys.length > 32)
        context.addIssue({
          code: 'custom',
          path: [],
          message: 'Runtime binding parameters cannot contain more than 32 keys.',
        });
      keys.forEach((key) => {
      ...
      const visit = (entry: unknown, path: PropertyKey[]) => {
        if (typeof entry === 'string' && entry.length > 2048)
          context.addIssue({
            code: 'custom',
            path,
            message: 'Runtime parameter string values cannot exceed 2048 characters.',
          });
        else if (Array.isArray(entry)) entry.forEach((child, index) => visit(child, [...path, index]));
...
export const DeclarativeRuntimeActionExecuteResultSchema = z
  .object({
    ...
    downloadError: z
      .object({ code: z.enum(['DECLARATIVE_DOWNLOAD_FORBIDDEN', 'DECLARATIVE_DOWNLOAD_UNAVAILABLE']) })
      .strict()
      .optional(),
  ...
  .superRefine((value, context) => {
    if (value.download && value.downloadError)
      context.addIssue({
        code: 'custom',
        path: ['downloadError'],
        message: 'A download response cannot contain both a target and an error.',
      });
  });
  ...
  .superRefine((value, context) => {
    if (value.activeReleaseRevisionRef.kind !== 'navigation-release') {
      context.addIssue({
        code: 'custom',
        path: ['activeReleaseRevisionRef', 'kind'],
        message: 'Navigation Action requests require an exact navigation-release revision.',
      });
    }
    if (value.navigationRevisionRef.kind !== 'navigation') {
      context.addIssue({
        ...
        message: 'Navigation Action requests require an exact navigation revision.',
      });
    }
  });
  ...
  .superRefine((value, context) => {
    ...
    if (
      ...
      context.addIssue({
        ...
        message: 'The exact environment revision must identify the selected environment.',
      });
    }
  });
    ...
    value.schemaDocuments.forEach((entry, index) => {
      ...
      if (documents.has(key)) {
        context.addIssue({
          ...
          message: 'Capability schema documents must be unique by exact RevisionRef.',
        });
      }
  ...
  .superRefine((value, context) => {
    if (
      ...
      context.addIssue({
        ...
        message: 'A Design catalog artifact must be a global monkeys-design release revision.',
      });
    }
    ...
    if (ids.some((id, index) => index > 0 && ids[index - 1].localeCompare(id) > 0)) {
      context.addIssue({
        code: 'custom',
        path: ['entries'],
        message: 'Design catalog entries must use deterministic capability identity order.',
      });
    }
  });
export type DesignCapabilityCatalogEntry = z.infer<typeof DesignCapabilityCatalogEntrySchema>;
export type DesignCapabilityCatalogArtifact = z.infer<typeof DesignCapabilityCatalogArtifactSchema>;
  ...
  .superRefine((value, context) => {
    ...
    if (ownerRepo === 'monkeys-design') {
      context.addIssue({
        ...
        message: 'Monkeys Design capabilities must come from the Design capability catalog.',
      });
    }
    if (
      ...
      String(registration.capabilityRevisionRef.revision) !== String(value.manifest.capabilityVersion) ||
      registration.capabilityRevisionRef.visibility !== 'global'
    ) {
      context.addIssue({
        code: 'custom',
        path: ['declarativeRegistration', 'capabilityRevisionRef'],
        message: 'A product capability must pin its exact global product-owned manifest revision.',
      });
    }
    if (
      ...
      String(registration.providerRevisionRef.revision) !== String(value.provider.providerVersion) ||
      registration.providerRevisionRef.visibility !== 'global'
    ) {
      context.addIssue({
        code: 'custom',
        path: ['declarativeRegistration', 'providerRevisionRef'],
        message: 'A product capability must pin its exact global product-owned provider revision.',
      });
    }
    if (
      ...
      String(value.provider.capabilityRef.version) !== String(value.manifest.capabilityVersion)
    ) {
      context.addIssue({
        code: 'custom',
    ...
    const providerBoundByManifest = value.manifest.runtime.providerBindings.some(
      (binding) =>
        ...
        String(binding.providerRef.version) === String(value.provider.providerVersion),
    );
    if (!providerBoundByManifest) {
      context.addIssue({
        code: 'custom',
        path: ['manifest', 'runtime', 'providerBindings'],
        message: 'The product capability manifest must bind the declared product provider.',
      });
    }
    if (
      registration.registrationRevisionRef.kind !== 'product-capability-registration' ||
      registration.registrationRevisionRef.ownerRepo !== ownerRepo ||
      registration.registrationRevisionRef.visibility !== 'global' ||
      registration.registrationRevisionRef.contentHash !== registration.sourceContentHash
    ) {
      context.addIssue({
        code: 'custom',
        path: ['declarativeRegistration', 'registrationRevisionRef'],
        message: 'A product capability registration must have exact product-owned release provenance.',
      });
    }
    registration.schemaDocuments.forEach((schema, index) => {
      if (schema.schemaRevisionRef.ownerRepo !== ownerRepo || schema.schemaRevisionRef.visibility !== 'global') {
        context.addIssue({
          code: 'custom',
          path: ['declarativeRegistration', 'schemaDocuments', index],
          message: 'Embedded product capability Schemas require exact global product authority.',
        });
      }
    });
    ...
    if (
      ...
      context.addIssue({
        ...
        message: 'Product capability permission codes must be unique and deterministically sorted.',
      });
    }
  });
export type ProductDeclarativeCapabilityRegistration = z.infer<typeof ProductDeclarativeCapabilityRegistrationSchema>;
...
export const DeclarativeCreateTemplateRegistrationSchema = z
  .object({
    ...
    description: I18nTextSchema,
    supportedSurfaces: z.array(ProductSurfaceSchema).min(1),
    legacyMigrationSourcePageIds: z.array(ContractIdentifierSchema).min(1).max(256).optional(),
  ...
  .superRefine((value, context) => {
    ...
    if (value.templateRevisionRef.kind !== 'declarative-create-template')
      context.addIssue({
        ...
        message: 'Create templates require declarative-create-template RevisionRefs.',
      });
    if (value.legacyMigrationSourcePageIds && value.resourceKind !== 'page') {
      context.addIssue({
        code: 'custom',
        path: ['legacyMigrationSourcePageIds'],
        message: 'Only Page templates can declare legacy Page migration sources.',
      });
    }
    if (
      value.legacyMigrationSourcePageIds &&
      new Set(value.legacyMigrationSourcePageIds).size !== value.legacyMigrationSourcePageIds.length
    ) {
      context.addIssue({
        code: 'custom',
        path: ['legacyMigrationSourcePageIds'],
...
export const DeclarativeProductResourceRegistrationSchema = z
  ...
  .superRefine((value, context) => {
    ...
    if (value.resourceKind === 'shell') {
      ...
      if (parsed.success) {
        const declaredSurfaces = new Set(parsed.data.surfaceChrome.map((entry) => entry.surface));
  ...
  .superRefine((value, context) => {
    if (value.definitionRevisionRef.kind !== 'domain-query-definition') {
      context.addIssue({
        ...
        message: 'A Domain Query registration must pin a domain-query-definition revision.',
      });
    }
    if (value.definitionRevisionRef.id !== value.definition.queryId) {
      context.addIssue({
        ...
        message: 'A Domain Query registration identity must match its definition.',
      });
    }
  });
  ...
  .superRefine((value, context) => {
    if (value.definitionRevisionRef.kind !== 'domain-command') {
      context.addIssue({
        code: 'custom',
        path: ['definitionRevisionRef', 'kind'],
        message: 'A Domain Command registration must pin a domain-command revision.',
      });
    }
    if (value.definitionRevisionRef.id !== value.definition.commandName) {
      context.addIssue({
        code: 'custom',
        path: ['definitionRevisionRef', 'id'],
        message: 'A Domain Command registration identity must match its definition.',
      });
    }
  });
...
export const ProductDeclarativeAuthoringCatalogSchema = z
  .object({
    ...
    domainQueryDefinitions: z.array(DeclarativeDomainQueryDefinitionRegistrationSchema).default([]),
    domainCommandDefinitions: z.array(DeclarativeDomainCommandDefinitionRegistrationSchema).default([]),
    schemaDefinitions: z.array(DeclarativeSchemaDefinitionRegistrationSchema).default([]),
    capabilityRegistrations: z.array(ProductDeclarativeCapabilityRegistrationSchema).default([]),
  })
  .strict()
  .superRefine((value, context) => {
    const duplicates = (items: readonly string[]) => items.find((item, index) => items.indexOf(item) !== index);
    const duplicateTemplate = duplicates(
      value.createTemplates.map((item) => `${item.templateRevisionRef.id}@${item.templateRevisionRef.revision}`),
    );
    ...
    const duplicateRouteSpace = duplicates(
      value.routeSpaces.map((item) => `${item.routeSpaceRevisionRef.id}@${item.routeSpaceRevisionRef.revision}`),
    );
    ...
    const duplicateQueryDefinition = duplicates(
      value.domainQueryDefinitions.map(
        (item) => `${item.definitionRevisionRef.id}@${item.definitionRevisionRef.revision}`,
      ),
    );
    ...
    const duplicateSchemaDefinition = duplicates(
      value.schemaDefinitions.map((item) => `${item.definitionRevisionRef.id}@${item.definitionRevisionRef.revision}`),
    );
    ...
    value.defaultPolicies.forEach((selection, index) => {
      ...
      if (
        !value.resourceRegistrations.some(
          (registration) =>
            registration.resourceRevisionRef.kind === selection.resourceRevisionRef.kind &&
            registration.resourceRevisionRef.id === selection.resourceRevisionRef.id &&
            registration.resourceRevisionRef.revision === selection.resourceRevisionRef.revision &&
            registration.resourceRevisionRef.contentHash === selection.resourceRevisionRef.contentHash,
        )
      ) {
        context.addIssue({
          code: 'custom',
          path: ['defaultPolicies', index, 'resourceRevisionRef'],
          message: 'Default policy must select an exact registered product resource.',
        });
      }
    });
  });
...
>;
export type DeclarativeSchemaDefinitionRegistration = z.infer<typeof DeclarativeSchemaDefinitionRegistrationSchema>;
  ...
  .superRefine((value, context) => {
    if (Number(Boolean(value.templateRevisionRef)) + Number(Boolean(value.sourceRevisionRef)) !== 1) {
      context.addIssue({
        ...
        message: 'Create intent requires exactly one governed template or source revision.',
      });
    }
  });
...
export const DeclarativeAuthoringChoicePageSchema = z.discriminatedUnion('choiceKind', [
  authoringChoicePage('capability', DeclarativeCapabilityAuthoringChoiceSchema),
  ...
  authoringChoicePage('workbench-target', DeclarativeWorkbenchTargetChoiceSchema),
]);
...
export type DeclarativeAuthoringPageInfo = z.infer<typeof DeclarativeAuthoringPageInfoSchema>;
export type DeclarativeAuthoringChoiceKind = z.infer<typeof DeclarativeAuthoringChoiceKindSchema>;
export type DeclarativeAuthoringChoicePage = z.infer<typeof DeclarativeAuthoringChoicePageSchema>;
export type DeclarativeAuthoringResourcePageQuery = z.infer<typeof DeclarativeAuthoringResourcePageQuerySchema>;
export type DeclarativeAuthoringChoicePageQuery = z.infer<typeof DeclarativeAuthoringChoicePageQuerySchema>;
...
const declarativeReferenceTenantScopeKey = (scope: z.infer<typeof TenantScopeSchema>): string => JSON.stringify(scope);
...
export const DeclarativeAuthoringResourceSummarySchema = z
  ...
  .strict()
  .superRefine((value, context) => {
    const expectedDefinitionKind = DeclarativeDefinitionKindByResourceKind[value.resourceKind];
    const expectedReleaseKind = DeclarativeReleaseKindByResourceKind[value.resourceKind];
    const expectedTenantScope = declarativeReferenceTenantScopeKey(value.tenantScope);
    const seenReleaseRefs = new Set<string>();

    value.releasePlacements.forEach((placement, index) => {
      const releasePath = ['releasePlacements', index, 'releaseRevisionRef'] as const;
      const definitionPath = ['releasePlacements', index, 'definitionRevisionRef'] as const;
      ...
      if (
        ...
        declarativeReferenceTenantScopeKey(placement.releaseRevisionRef.tenantScope!) !== expectedTenantScope
      ) {
        context.addIssue({
          code: 'custom',
      ...
      if (seenReleaseRefs.has(releaseIdentity)) {
        context.addIssue({
          code: 'custom',
          path: [...releasePath],
          message: 'Release placements must not repeat an exact release revision.',
        });
      }
      ...
      if (placement.definitionRevisionRef.id !== value.resourceId) {
        context.addIssue({
          ...
          message: 'Release placement definition must reference the summary resource.',
        });
      }
      if (
        placement.definitionRevisionRef.visibility !== 'tenant' ||
        declarativeReferenceTenantScopeKey(placement.definitionRevisionRef.tenantScope!) !== expectedTenantScope
      ) {
        context.addIssue({
          code: 'custom',
          path: [...definitionPath, 'tenantScope'],
          message: 'Release placement definition must use the resource tenant scope.',
        });
      }
    });
  });
...
export type DeclarativeAuthoringResourcePage = z.infer<typeof DeclarativeAuthoringResourcePageSchema>;
...
export const DeclarativePublicationItemResultSchema = z
  .object({
    ...
    bundle: z.union([PageRuntimeBundleSchema, NavigationRuntimeBundleSchema, WorkbenchRuntimeBundleSchema]).optional(),
  })
  .strict()
  .superRefine((value, context) => {
    ...
    if (
      ...
      context.addIssue({
        ...
        message: 'A publication item must return the exact release identity for its resource kind.',
      });
    }
    const definitionRef =
    ...
    if (definitionRef.id !== value.resourceId) {
      context.addIssue({
        code: 'custom',
        path: ['resourceId'],
        message: 'A publication item release must identify the requested resource.',
      });
    }
    const unavailable = value.release.operation === 'deactivate' || Boolean(value.release.legacyRestoration);
    ...
    if (value.bundle) {
      ...
      const sameRevision = (left: z.infer<typeof RevisionRefSchema>, right: z.infer<typeof RevisionRefSchema>) =>
      ...
      if (
        ...
        !sameRevision(value.bundle.releaseRevisionRef, value.releaseRevisionRef) ||
        !sameRevision(bundleDefinitionRef, definitionRef)
      ) {
        context.addIssue({
          code: 'custom',
          path: ['bundle'],
          message: 'A publication item Runtime Bundle must pin the exact returned Release and definition revisions.',
        });
      }
    }
  });
...
const DeclarativeDirectPublicationResultSchema = z
  ...
  .strict()
  .superRefine((value, context) => {
    const expectedOperation = value.action === 'publish' ? 'activate' : 'rollback';
    value.items.forEach((item, index) => {
      if (item.release.operation !== expectedOperation) {
        context.addIssue({
          code: 'custom',
          path: ['items', index, 'release', 'operation'],
          message: 'A direct publication result must match its requested action.',
        });
      }
    });
  });
  ...
  .superRefine((value, context) => {
    if (value.publicationPlanRevisionRef.kind !== 'publication-plan') {
      context.addIssue({
        ...
        message: 'An atomic publication result must pin its Publication Plan revision.',
      });
    }
  });
...
export const DeclarativeLegacyMigrationInspectionSchema = z
  ...
  .strict()
  .superRefine((value, context) => {
    const expectedKind = value.migrationKind === 'workbench' ? 'workbench' : 'page';
    ...
    if (
      value.takeoverAuthorization.inspectedSourceContentHash !== value.sourceContentHash ||
      value.takeoverAuthorization.normalizedPath !== value.normalizedPath ||
      value.takeoverAuthorization.targetResourceRef.kind !== value.targetResourceRef.kind ||
      value.takeoverAuthorization.targetResourceRef.id !== value.targetResourceRef.id ||
      value.takeoverAuthorization.targetResourceRef.ownerRepo !== value.targetResourceRef.ownerRepo
    ) {
      context.addIssue({
        code: 'custom',
        path: ['takeoverAuthorization'],
        message: 'The takeover authority must bind the exact inspected source, path and target.',
      });
    }
    if (value.canCreateDraft !== (value.blockers.length === 0)) {
      context.addIssue({
        code: 'custom',
        path: ['canCreateDraft'],
        message: 'Draft creation is allowed exactly when inspection has no blockers.',
      });
    }
  });
export type DeclarativeLegacyMigrationInspection = z.infer<typeof DeclarativeLegacyMigrationInspectionSchema>;
...
export const DeclarativeAuthoringBootstrapSchema = z
  .object({
    ...
    deferredChoiceKinds: z.array(DeclarativeAuthoringChoiceKindSchema).optional(),
...
export const DeclarativeRuntimeResolvedSchema = z
  ...
  .strict()
  .superRefine((value, context) => {
    const outcomes = [value.page, value.workbench, value.redirect].filter(Boolean).length;
    if (outcomes !== 1)
      context.addIssue({
        code: 'custom',
        path: ['page'],
        message: 'A resolved runtime must contain exactly one Page, Workbench, or governed redirect outcome.',
      });
  });
  ...
  .superRefine((value, context) => {
    if (value.legacyAdapterRevisionRef.kind !== 'legacy-route-adapter') {
      context.addIssue({
        ...
        message: 'A legacy runtime outcome must pin a legacy-route-adapter revision.',
      });
    }
  });
...
const DeclarativeRuntimeAuthorizedOutcomeSchema = z
  ...
  .strict()
  .superRefine((value, context) => {
    if (value.resolved.surface !== value.surface || value.resolved.normalizedPath !== value.normalizedPath) {
      context.addIssue({
        code: 'custom',
        path: ['resolved'],
        message: 'An authorized declarative outcome must match its safe route envelope.',
      });
    }
  });

const DeclarativeRuntimeDeniedOutcomeSchema = (outcome: 'forbidden' | 'unavailable' | 'not-found', code: string) =>
  z
    .object({
...
const DeclarativeRuntimeForbiddenOutcomeSchema = z.discriminatedUnion('resourceKind', [
  z
    .object({
    ...
    .strict()
    .superRefine((value, context) => {
      if (value.presentation && value.presentation.surface !== value.surface) {
        context.addIssue({
          code: 'custom',
          path: ['presentation', 'surface'],
          message: 'A forbidden Page presentation must match the resolved route surface.',
        });
      }
    }),
  z
...
export const DeclarativeRuntimeResolveResultSchema = z.union([
  ...
  DeclarativeRuntimeDeniedOutcomeSchema('unavailable', 'DECLARATIVE_ROUTE_UNAVAILABLE'),
  DeclarativeRuntimeDeniedOutcomeSchema('not-found', 'DECLARATIVE_ROUTE_NOT_FOUND'),
]);
export type DeclarativeRuntimeResolveResult = z.infer<typeof DeclarativeRuntimeResolveResultSchema>;
        ...
        .superRefine((value, context) => {
          if (value.authority === 'legacy') {
            if (value.releaseSlotId || value.activeReleaseRevisionRef || value.bundle) {
              context.addIssue({
                code: 'custom',
                path: ['authority'],
                message: 'Legacy navigation placements cannot expose declarative Release or Bundle state.',
              });
            }
            if (value.state !== 'active')
              context.addIssue({
                code: 'custom',
                path: ['state'],
                message: 'Legacy navigation placements are active until an explicit declarative cutover.',
              });
            return;
          }
          if (!value.releaseSlotId || !value.activeReleaseRevisionRef) {
            context.addIssue({
              code: 'custom',
              path: ['releaseSlotId'],
              message: 'Declarative navigation placements require an exact active Release reference.',
            });
          }
          if (value.state === 'active' && !value.bundle) {
            context.addIssue({
              code: 'custom',
              path: ['bundle'],
              message: 'An active declarative navigation placement requires its identity-filtered Runtime Bundle.',
            });
          }
          if (value.state === 'deactivated' && value.bundle) {
            context.addIssue({
              code: 'custom',
              path: ['bundle'],
              message: 'A deactivated declarative navigation placement cannot expose a Runtime Bundle.',
            });
          }
          if (value.bundle && (value.bundle.placement !== value.placement || value.bundle.surface !== value.surface)) {
            context.addIssue({
              code: 'custom',
              path: ['bundle'],
              message: 'The Navigation Runtime Bundle must match its placement and surface.',
            });
          }
        }),
    ),
  ...
  .superRefine((value, context) => {
    ...
    if (active.length !== value.bundles.length || active.length !== value.releaseSlotIds.length) {
      context.addIssue({
        code: 'custom',
        path: ['placements'],
        message: 'Compatibility bundle arrays must exactly project active declarative placements.',
      });
      return;
    }
    active.forEach((entry, index) => {
      if (
        ...
        context.addIssue({
          ...
          message: 'Navigation placement and compatibility bundle order must match.',
        });
      }
    });
  });
        ...
        .superRefine((value, context) => {
          if (value.authority === 'legacy' && value.visible)
            context.addIssue({
              code: 'custom',
              path: ['visible'],
              message: 'Legacy workbenches are delivered by the compatibility API, not by declarative summaries.',
            });
      ...
      .superRefine((value, context) => {
        if (value.authority === 'declarative' && (!value.releaseSlotId || !value.activeReleaseRevisionRef)) {
          context.addIssue({
            code: 'custom',
            path: ['releaseSlotId'],
            message: 'Declarative Workbench selection requires its exact Release slot and revision.',
          });
        }
        if (value.authority === 'legacy' && (value.releaseSlotId || value.activeReleaseRevisionRef)) {
          context.addIssue({
            code: 'custom',
            path: ['releaseSlotId'],
            message: 'Legacy Workbench selection cannot expose declarative Release state.',
          });
        }
      })
      .nullable(),
...
export const DeclarativeRuntimeRouteOwnerSchema = z
  .object({
    ...
    owner: z.discriminatedUnion('authority', [DeclarativeLegacyRouteOwnerSchema, DeclarativeReleaseRouteOwnerSchema]),
    contentHash: Sha256Schema,
  })
  .strict();
export type DeclarativeRuntimeRouteOwner = z.infer<typeof DeclarativeRuntimeRouteOwnerSchema>;

export type DeclarativeRuntimeResolved = z.infer<typeof DeclarativeRuntimeResolvedSchema>;
export type DeclarativeRuntimeRedirect = z.infer<typeof DeclarativeRuntimeRedirectSchema>;
